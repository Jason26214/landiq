...
import { FileText, Download, Share2, BarChart3, AlertTriangle, CheckCircle2, TrendingUp } from "lucide-react";
import { SlideLayout } from "../shared/SlideLayout";
import { FadeIn } from "../shared/FadeIn";

const reportSections = [
  { title: "Executive Summary", status: "complete", detail: "Site overview, key metrics, go/no-go recommendation" },
  { title: "Planning Analysis", status: "complete", detail: "Zoning SP5, FSR 8:1, Height SEPP-controlled, Heritage I2287" },
  { title: "Development Potential", status: "complete", detail: "Max GFA 82,628 sqm · Est. 480 apartments · 95% efficiency" },
  { title: "Risk Assessment", status: "warning", detail: "Heritage overlay detected · Flood: Clear · Bushfire: Clear" },
  { title: "Financial Feasibility", status: "complete", detail: "GRV $384M · TDC $298M · Margin 28.8% · IRR 24.2%" },
  { title: "Recommendation", status: "complete", detail: "PROCEED — Strong yield, manageable heritage constraint" },
];
          ...
          <FadeIn delay={0.6}>
            <div className="space-y-5">
              {[
                ...
                { label: "Est. Dwellings", value: "480", unit: "apartments", icon: TrendingUp },
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    <metric.icon className="w-4 h-4 text-primary-600" strokeWidth={1.5} />
                    <span className="font-sans text-xs uppercase tracking-[0.06em] text-surface-500">
                      {metric.label}
                    </span>
                  </div>
                  <div className="flex items-baseline gap-1.5">
                    <span className="font-serif text-3xl text-primary-900">{metric.value}</span>
                    <span className="font-sans text-sm text-surface-500">{metric.unit}</span>
                  </div>
                </motion.div>
              ))}
            </div>
